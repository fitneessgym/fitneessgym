...
async function applySite(){
 ...
 set('galleryTitle',s.galleryTitle);set('galleryNote',s.galleryNote);const gal=document.getElementById('galleryList');if(gal)gal.innerHTML=s.gallery.map(x=>`<div>${escSite(x)}</div>`).join('');
 ...
 const wa=document.getElementById('whatsappLink');if(wa)wa.href='https://wa.me/'+s.whatsapp.replace(/\D/g,'');
}
