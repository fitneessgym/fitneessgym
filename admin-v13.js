(() => {
  ...
  function renderNutrition(){
    ...
    // Build the table from customer_nutrition first. This table is the source
    // of truth for the saved calories/macros. The profile table is only used
    // for BMR/TDEE and personal details when available.
    const byId=new Map();
    customerNutrition.forEach(n=>{
      if(n && n.customer_id!=null) byId.set(String(n.customer_id),n);
    });
    nutritionProfiles.forEach(p=>{
      if(p && p.customer_id!=null && !byId.has(String(p.customer_id))) byId.set(String(p.customer_id),{});
    });

    let ids=[...byId.keys()];
    if(filter) ids=ids.filter(id=>id===String(filter));

    // Also include customers that have a profile even if nutrition rows are
    // temporarily unavailable, so the selected player never produces a blank table.
    if(filter && !ids.includes(String(filter))){
      const c=getCustomer(filter);
      if(c) ids=[String(filter)];
    }

    const rows=ids.map(id=>{
      const c=getCustomer(id)||{id,name:id};
      const p=nutritionProfiles.find(x=>String(x.customer_id)===String(id))||{};
      const n=customerNutrition.find(x=>String(x.customer_id)===String(id))||{};
      return {c,p,n};
    }).filter(x=>!filter || x.c);

    body.innerHTML=rows.map(({c,p,n})=>{
      const calories=n.daily_calories ?? p.daily_calories ?? p.target_calories;
      const protein=n.protein_g ?? p.protein_g;
      const carbs=n.carbs_g ?? p.carbs_g;
      const fats=n.fat_g ?? n.fats_g ?? p.fat_g ?? p.fats_g;
      const goal=n.fitness_goal ?? n.goal ?? p.fitness_goal ?? p.goal;
      const bmr=p.bmr ?? n.bmr;
      const tdee=p.tdee ?? n.tdee;
      return `<tr><td><b>${esc(fullName(c))}</b></td><td>${bmr!=null?Math.round(Number(bmr)): '—'}</td><td>${tdee!=null?Math.round(Number(tdee)): '—'}</td><td><b>${calories!=null?Math.round(Number(calories)):'—'}</b></td><td>${esc(goalLabel(goal))}</td><td>${protein!=null?Math.round(Number(protein))+'غ':'—'}</td><td>${carbs!=null?Math.round(Number(carbs))+'غ':'—'}</td><td>${fats!=null?Math.round(Number(fats))+'غ':'—'}</td></tr>`;
    }).join('')||'<tr><td colspan="8" class="empty">لا توجد ملفات تغذية محفوظة لهذا اللاعب.</td></tr>';
  }
  $('nutritionCustomer')?.addEventListener('change',()=>{
    ...
    $('nutritionSex').value=p.sex||'male'; $('nutritionAge').value=p.age||''; $('nutritionWeight').value=p.weight||''; $('nutritionHeight').value=p.height||''; $('nutritionActivity').value=p.activity_level||1.55; $('nutritionBodyType').value=p.body_type||'mesomorph'; $('nutritionGoal').value=p.goal||'build';
  });
  $('nutritionFilterCustomer')?.addEventListener('change',renderNutrition);
  $('nutritionForm')?.addEventListener('submit',async e=>{
    e.preventDefault();
    const customerId=$('nutritionCustomer').value, sex=$('nutritionSex').value, age=Number($('nutritionAge').value), weight=Number($('nutritionWeight').value), height=Number($('nutritionHeight').value), activity=Number($('nutritionActivity').value), bodyType=$('nutritionBodyType').value, goal=$('nutritionGoal').value;
