(() => {
  ...
  $('workoutLogForm')?.addEventListener('submit',async e=>{
    ...
    const payload={customer_id:customerId,workout_title:title,workout_day:day,workout_date:$('logDate').value||today(),sets_completed:sets,reps:$('logReps').value.trim(),weight,duration:$('logDuration').value.trim(),notes:$('logNotes').value.trim()}; // V9: never send created_by
